...
def data_send_info_window(window: Tk):
    ...
    tree = ttk.Treeview(window, columns=("Column1", "Column2", "Column3", "Column4"), show="headings")

    # Define column headings and set column width
    tree.heading("Column1", text="src IP")
    tree.heading("Column2", text="dst IP")
    tree.heading("Column3", text="src PORT")
    tree.heading("Column4", text="dst PORT")

    tree.column("Column1", width=200)
    tree.column("Column2", width=200)
    tree.column("Column3", width=200)
    tree.column("Column4", width=200)

    tree.place(relx=0.5, rely=0.52, height=450, anchor="center")
    refresh_send_data_in_db(tree)

    refresh_send_db_btn = tk.Button(window, text="Refresh", command=lambda: (refresh_send_data_in_db(tree)), width=15, height=2, font=("Helvetica", 12))
    ...
    destroy_widgets(data_send_window_text, tree, refresh_send_db_btn, text_of_function_send_db, btn_exit_send_data), main.create_main_menu(window)), width=15,
...
def create_block_ip_window(window: Tk):
    ...
    # Create an "Add" button to add entered text to the listbox
